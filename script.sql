...
SELECT restaurant.name AS name, address.street_number AS line_1, address.street_name AS street, restaurant.phone AS telephone
FROM restaurant, address
WHERE restaurant.id = address.restaurant_id;

SELECT MAX(stars) AS best_rating FROM review;
...
SELECT stars AS best_rating, description 
FROM review
WHERE
  stars = (SELECT MAX(stars) FROM review);
